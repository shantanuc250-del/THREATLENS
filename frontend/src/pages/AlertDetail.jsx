...
const STATUS_OPTIONS = [
  { id: 'NEW', label: 'NEW', color: 'bg-purple-500/15 text-info border-purple-500/30' },
  { id: 'INVESTIGATING', label: 'INVESTIGATING', color: 'bg-warn-soft text-warn border-warn-soft' },
  { id: 'RESOLVED', label: 'RESOLVED', color: 'bg-ok-soft text-ok border-ok-soft' },
  { id: 'FALSE_POSITIVE', label: 'FALSE POSITIVE', color: 'bg-card-hover text-body border-line-strong' },
];
  ...
  if (error || !alert) {
    return (
      <div className="glass-card p-8 max-w-lg mx-auto my-12 text-center space-y-4">
        <div className="w-12 h-12 rounded-full bg-danger-soft border border-danger-soft text-danger flex items-center justify-center mx-auto">
          <AlertTriangle size={24} />
        </div>
        <h3 className="text-lg font-bold text-hi">Alert Not Found</h3>
    ...
    <div className="animate-fadeIn space-y-6 w-full max-w-full min-w-0">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-line pb-4">
        <div className="flex items-center gap-3 min-w-0">
          <button onClick={() => navigate('/alerts')} className="btn-secondary px-3 py-2 text-body">
            <ArrowLeft size={16} />
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-hi tracking-tight flex items-center gap-2">
                <Shield size={20} className="text-brand-2" />
      ...
      {feedback && (
        <div className={`p-3 rounded-lg text-xs font-semibold flex items-center justify-between ${
          feedback.isError ? 'bg-danger-soft border border-danger-soft text-danger' : 'bg-ok-soft border border-ok-soft text-ok'
        }`}>
          <span>{feedback.msg}</span>
          <button onClick={() => setFeedback(null)} className="text-muted hover:text-hi">✕</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_380px] lg:grid-cols-[minmax(0,1fr)_320px] gap-6 w-full min-w-0">
        ...
        <div className="space-y-6 min-w-0">
          ...
          <div className="glass-card p-5">
            <h3 className="text-sm font-semibold text-hi mb-4 flex items-center gap-2 border-b border-line pb-2.5">
              <Brain size={16} className="text-brand" /> Alert ML Summary
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="bg-surface rounded-lg p-3.5 border border-line">
                <p className="text-xs text-muted">Prediction</p>
                <p className={`text-lg font-bold mt-1 ${isAttack ? 'text-danger' : 'text-ok'}`}>
              ...
              <div className="bg-surface rounded-lg p-3.5 border border-line">
                ...
                <p className={`text-lg font-bold mt-1 font-mono ${
                  prob > 0.85 ? 'text-danger' : prob > 0.6 ? 'text-warn' : 'text-brand'
                }`}>
                  {(prob * 100).toFixed(1)}%
                </p>
              </div>
              ...
              <div className="bg-surface rounded-lg p-3.5 border border-line">
                <p className="text-xs text-muted">Attack Type</p>
                <p className="text-sm font-semibold text-body mt-1 truncate">
              ...
              <div className="bg-surface rounded-lg p-3.5 border border-line">
                ...
                <p className="text-sm font-mono text-brand-2 mt-1">
          ...
          <div className="glass-card p-5">
            <h3 className="text-sm font-semibold text-hi mb-4 flex items-center gap-2 border-b border-line pb-2.5">
              <Network size={16} className="text-brand-2" /> Traffic Information
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-surface p-3 rounded-lg border border-line">
                <p className="text-xs text-muted flex items-center gap-1">
                  <MapPin size={12} className="text-faint" /> Source IP
                </p>
                <p className="text-sm text-hi font-mono font-medium mt-1 truncate">
              ...
              <div className="bg-surface p-3 rounded-lg border border-line">
                <p className="text-xs text-muted flex items-center gap-1">
                  <Server size={12} className="text-faint" /> Destination IP
                </p>
                <p className="text-sm text-hi font-mono font-medium mt-1 truncate">
              ...
              <div className="bg-surface p-3 rounded-lg border border-line">
                <p className="text-xs text-muted">Protocol</p>
              ...
              <div className="bg-surface p-3 rounded-lg border border-line">
                <p className="text-xs text-muted">Source Bytes</p>
                <p className="text-sm text-body font-mono mt-1">
              ...
              <div className="bg-surface p-3 rounded-lg border border-line">
                <p className="text-xs text-muted">Destination Bytes</p>
                <p className="text-sm text-body font-mono mt-1">
              ...
              <div className="bg-surface p-3 rounded-lg border border-line">
                <p className="text-xs text-muted">Connection Count</p>
                <p className="text-sm text-body font-mono mt-1">
          ...
          {importances.length > 0 && (
            <div className="glass-card p-5">
              <h3 className="text-sm font-semibold text-hi mb-2 flex items-center gap-2">
                <Brain size={16} className="text-info" /> Model Feature Importance Breakdown
              </h3>
                ...
                {importances.slice(0, 8).map((fi, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3 text-xs">
                      <span className="text-body w-36 sm:w-44 text-right font-mono truncate shrink-0">
                        {fi.feature}
                      </span>
                      <div className="flex-1 bg-surface rounded-full h-2 overflow-hidden border border-line">
                        <div
                          className="h-full bg-brand-gradient rounded-full transition-all duration-500"
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                      <span className="text-muted font-mono w-14 text-right shrink-0">
                        {pct.toFixed(1)}%
                      </span>
                    </div>
                  );
          ...
          {/* Disclaimer Banner */}
          <div className="p-3.5 rounded-lg bg-warn-soft border border-warn-soft text-xs text-warn flex items-start gap-2.5">
          ...
          {Object.keys(rawFeatures).length > 0 && (
            <div className="glass-card p-5">
              <h3 className="text-sm font-semibold text-hi mb-3">All Raw Network Features</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2.5 max-h-60 overflow-y-auto pr-1">
                {Object.entries(rawFeatures).map(([k, v]) => (
                  <div key={k} className="bg-surface p-2.5 rounded border border-line">
                    <p className="text-[0.65rem] text-faint truncate" title={k}>{k}</p>
                    <p className="text-xs font-mono text-body truncate mt-0.5">{String(v)}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="space-y-6 min-w-0">
          ...
          <div className="glass-card p-5 sticky top-20">
            <h3 className="text-sm font-bold text-hi mb-4 flex items-center gap-2 border-b border-line pb-2.5">
              <FileText size={16} className="text-brand" /> 📄 Analyst Workflow
            </h3>
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3.5 py-2.5 rounded-lg text-xs font-bold transition-all flex items-center justify-between border ${
                      isSelected
                        ? `${opt.color} shadow-sm`
                        : 'bg-surface text-muted border-line hover:border-line-strong hover:text-body'
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
                      <span className={`w-2 h-2 rounded-full ${isSelected ? 'bg-current' : 'bg-card-hover'}`} />
                      <span>{opt.label}</span>
                    </div>
            ...
            <div className="bg-surface border border-warn-soft rounded-lg p-4 mb-6">
              ...
              <p className="text-[0.7rem] text-faint mt-2 leading-relaxed">
            ...
            <div>
              ...
              <textarea
                ...
                rows={5}
                className="input-field resize-none text-xs font-sans mb-3 bg-surface focus:bg-app-2"
